...
#include "stm32f4xx.h"
//Using only header files except fsmc
...
static void Init_GPIO(void)
{
	//clocks are already enabled in hal init
	//reset output, pull up...

	RCC ->AHB1ENR |= RCC_AHB1Periph_GPIOD | RCC_AHB1Periph_GPIOE
			| RCC_AHB1Periph_GPIOC;
	palSetPadMode(GPIOC, 6,
			PAL_STM32_PUDR_PULLUP | PAL_STM32_OSPEED_MID1 | PAL_STM32_OTYPE_PUSHPULL | PAL_STM32_MODE_OUTPUT);
	//fsmc
	palSetGroupMode(GPIOD,
			GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_14 | GPIO_Pin_15,
			0, PAL_MODE_ALTERNATE(GPIO_AF_FSMC));
	palSetGroupMode(GPIOE,
